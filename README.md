...
- ✅ **去文字 / 水印**:在帧画面上拖框圈选要去除的区域,支持三种方式:
  - **智能修复(delogo)**:邻域插值,适合纯色或简单背景的台标 / 水印
  - **模糊**:高斯模糊,适合人脸 / 隐私
  - **马赛克**:像素化,效果最明显
...
2. **去水印(可选)**:右上面板拖动滑块取一帧,在画面上 **按住鼠标拖动框选** 要去除的区域(可框多个),从下拉菜单选去除方式
3. **开始转写**:点字幕面板"开始转写",等待 1-3 分钟(10 分钟视频)
4. **编辑字幕**:勾选要保留的句子,直接点 textarea 修改文字
5. **配置规则**:开启"删除静默"、调整 padding;可选 BGM
6. **导出**:确认输出路径 → 点"开始导出"
